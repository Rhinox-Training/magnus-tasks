...
public class ValueReferenceLookupDrawer : BasePropertyDrawer<ValueReferenceLookup>
{
    ...
    {
        HostInfo.TryGetChild(nameof(ValueReferenceLookup.ValueResolversByKey), out TypedHostInfoWrapper<Dictionary<SerializableGuid, IValueResolver>> valueResolversByKey);
        ...
        for (int pageI = _pager.StartIndex; pageI < _pager.EndIndex; ++pageI)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                using (new EditorGUILayout.VerticalScope(GUILayout.Width(20)))
                {
                    ...
                    // Check usages
                    var hasUsageData = HostInfo.Parent.GetReturnType().EqualsOneOf(typeof(TaskObject), typeof(TaskEditViewPage));
                    if (hasUsageData && host != null && CustomEditorGUI.IconButton(UnityIcon.AssetIcon("Fa_Asterisk")))
                    {
