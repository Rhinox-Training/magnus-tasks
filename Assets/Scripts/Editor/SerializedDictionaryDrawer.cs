...
[CustomPropertyDrawer(typeof(KeyDictionary), true)]
    public class SerializedDictionaryDrawer : BasePropertyDrawer<TestDic>
    {
	    const string KeysFieldName = "_keyData";
		const string ValuesFieldName = "_valueData";
		protected const float IndentWidth = 15f;

		static GUIContent ICON_PLUS             = IconContent ("Toolbar Plus", "Add entry");
		static GUIContent ICON_MINUS            = IconContent ("Toolbar Minus", "Remove entry");
		static GUIContent WARNING_ICON_CONFLICT = IconContent ("console.warnicon.sml", "Conflicting key, this entry will be lost");
		static GUIContent WARNING_ICON_OTHER    = IconContent ("console.infoicon.sml", "Conflicting key");
		static GUIContent WARNING_ICON_NULL     = IconContent ("console.warnicon.sml", "Null key, this entry will be lost");
		static GUIStyle   BUTTON_STYLE          = GUIStyle.none;
		static GUIContent TEMP_CONTENT          = new GUIContent();
	...
	{
		var v = data as HostInfo;
		var property = v.Root.FindProperty(v.Path);
		...
		var buttonWidth = BUTTON_STYLE.CalcSize(ICON_PLUS).x;

		var labelPosition = position;
		labelPosition.height = EditorGUIUtility.singleLineHeight;
		if (property.isExpanded)
			labelPosition.xMax -= BUTTON_STYLE.CalcSize(ICON_PLUS).x;
		...
		if (property.isExpanded)
		{
			...
			if(GUI.Button(buttonPosition, ICON_PLUS, BUTTON_STYLE))
			{
			...
			foreach(var entry in EnumerateEntries(keyArrayProperty, valueArrayProperty))
			{
				...
				buttonPosition.height = EditorGUIUtility.singleLineHeight;
				if(GUI.Button(buttonPosition, ICON_MINUS, BUTTON_STYLE))
				{
				...
				if(i == conflictState.conflictIndex && conflictState.conflictOtherIndex == -1)
				{
					...
					iconPosition.size =  BUTTON_STYLE.CalcSize(WARNING_ICON_NULL);
					GUI.Label(iconPosition, WARNING_ICON_NULL);
				}
				else if(i == conflictState.conflictIndex)
				{
					var iconPosition = linePosition;
					iconPosition.size =  BUTTON_STYLE.CalcSize(WARNING_ICON_CONFLICT);
					GUI.Label(iconPosition, WARNING_ICON_CONFLICT);
				}
				else if(i == conflictState.conflictOtherIndex)
				{
					var iconPosition = linePosition;
					iconPosition.size =  BUTTON_STYLE.CalcSize(WARNING_ICON_OTHER);
					GUI.Label(iconPosition, WARNING_ICON_OTHER);
				}
