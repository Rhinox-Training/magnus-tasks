    ...
    public class TagContainerDrawer : OdinValueDrawer<TagContainer>
    {
        ...
        public static class Styles
        {
            ...
            public static GUIStyle TagCardStyle => _tagCardStyle ?? (_tagCardStyle =
                new GUIStyle(SirenixGUIStyles.CardStyle)
                {
            ...
            public static GUIStyle TagLabelStyle => _tagLabelStyle ?? (_tagLabelStyle =
                new GUIStyle(SirenixGUIStyles.CenteredWhiteMiniLabel)
                {
                ...
                using (new eUtility.VerticalGroup())
                {
                    ...
                    using (new eUtility.HorizontalGroup())
                    {
                        if (container.Tags.IsNullOrEmpty())
                            GUILayout.Label(GUIHelper.TempContent("Click the + to add some tags"),
                        ...
                        foreach (var tag in container.Tags)
                        {
                            ...
                            var width = Styles.TagCardStyle.CalcSize(GUIContent.none).x +
                                        Styles.TagLabelStyle.CalcSize(content).x;
        ...
        private void OpenSelector(Rect r, TagContainer container)
        {
            ...
            selector.SelectionConfirmed += x => { container.Tags.AddRange(x); };
        }
        ...
        {
            IEnumerable<ValueDropdownItem<string>> source =
                GetValues(container) ?? Enumerable.Empty<ValueDropdownItem<string>>();
