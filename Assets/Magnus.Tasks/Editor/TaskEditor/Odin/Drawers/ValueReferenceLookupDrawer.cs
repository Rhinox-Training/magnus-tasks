...
public class ValueReferenceLookupDrawer : SimpleOdinValueDrawer<ValueReferenceLookup>
{
    ...
    protected override void OnCustomDrawPropertyLayout(GUIContent label, IPropertyValueEntry<ValueReferenceLookup> valueEntry)
    {
        ...
        _expanded = SirenixEditorGUI.ToolbarToggle(_expanded, _expanded ? EditorIcons.TriangleDown : EditorIcons.TriangleRight);
    ...
    private void DrawKeyListButtons(object host)
    {
        ...
        {
            EditorInputDialog.Create("Create ReferenceKey", "Input the default name for the ReferenceKey:", "Add New Key")
        ...
        var content = GUIContentHelper.TempContent("Simplify", "If multiple keys refer to the same object, condenses those into 1 and rectifies all usages.");
        if (host != null && GUILayout.Button(content, SirenixGUIStyles.ButtonRight, GUILayoutOptions.ExpandWidth(false)))
        {
    ...
    private void DrawDefaults()
    {
        ...
        if (_selectedPropertyToInspect != null)
        {
            ...
            for (int i = _defaultsPager.StartIndex; i < _defaultsPager.EndIndex; ++i)
            {
                ...
                {
                    // Remove
                    ValueEntry.SmartValue.RemoveDefault(_selectedDefaultFilter, ((DefaultTypeReferenceKey)child.ValueEntry.WeakSmartValue).FieldData.Field);
