...
// [ShowOdinSerializedPropertiesInInspector]
    public class ValueReferenceLookup : IReferenceResolver
    {
        ...
        public bool RegisterDefault(FieldInfo field, SerializableGuid newDefaultGuid, bool overwriteIfExists = true)
        {
            ...
            if (valRefAttr == null)
            {
                PLog.Error<VortexLogger>(
                    $"Cannot RegisterDefault for field {field.Name} in {field.DeclaringType?.Name}, is not a ValueReference (attribute missing)");
                return false;
            }
            ...
            if (valRefAttr.TargetField != null)
            {
                ...
                {
                    PLog.Error<VortexLogger>(
                        $"Unable to RegisterDefault for field {field.Name}, TargetField not found ({valRefAttr.TargetField})");
                    return false;
                }
            }
            ...
            {
                PLog.Error<VortexLogger>(
                    $"Unable to RegisterDefault for field {field.Name}, ReferenceType specified by string instead of type (str: {valRefAttr.TypeLookup})");
                return false;
            }
        ...
        public SerializableGuid Register(SerializableGuid guid, string defaultName, IValueResolver resolver,
            bool overwriteOnNotNull = false)
        {
            if (resolver == null)
                throw new ArgumentException($"Cannot register '{defaultName}' with no resolver.");
        ...
        public SerializableGuid Register(string defaultName, Type objectType)
        {
            ...
            var registerMethod = methods.FirstOrDefault(x =>
                x.Name.Equals(nameof(Register)) && x.IsGenericMethod && x.GetParameters().Length == 1);
